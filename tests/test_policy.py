...
def action(**kw):
    base=dict(action_id='a1',action_type='update_bid',ad_product='SPONSORED_PRODUCTS',entity_type='keyword',entity_id='1',arguments={'field':'bid'},before={'bid':1.0},after={'bid':1.2},spend_delta=2,confidence=.9,evidence_refs=('e1',),dependencies=(),reversible=True,rollback={'bid':1.0},prewrite_observed_at=datetime.now(timezone.utc).isoformat())
...
def test_campaign_create_requires_paused():
    a=action(action_type='create_campaign',entity_type='campaign',entity_id='new',arguments={'state':'ENABLED','budget':50},before={},after={'budget':50},spend_delta=50)
    d=policy().evaluate_action(a); assert not d.allowed and any('PAUSED' in x for x in d.reasons)
