...
def action(**kw):
    base=dict(action_id='a1',action_type='update_bid',ad_product='SPONSORED_PRODUCTS',entity_type='keyword',entity_id='k1',arguments={'field':'bid'},before={'bid':1.0},after={'bid':1.1},spend_delta=0,confidence=.9,evidence_refs=('live:keyword:k1',),dependencies=(),reversible=True,rollback={'bid':1.0},prewrite_observed_at=NOW(),rationale='test')
...
def test_product_ad_create_requires_observed_asin():
    a=action(action_type='create_ad',entity_type='ad',entity_id='new',arguments={'advertisedAsin':'B000BAD999'},before={},after={},spend_delta=0)
    d=engine().evaluate_action(a,context=context()); assert not d.allowed and any('ASIN' in x for x in d.reasons)
    b=action(action_type='create_ad',entity_type='ad',entity_id='new2',arguments={'advertisedAsin':'B000TEST01'},before={},after={},spend_delta=0)
...
def test_pending_controller_campaign_cannot_activate(tmp_path:Path):
    ...
    a=action(action_type='enable_campaign',entity_type='campaign',entity_id='123',arguments={'state':'ENABLED'},before={'state':'PAUSED','name':'CODEX-test'},after={'state':'ENABLED'},spend_delta=0)
...
def test_profile_budget_expansion_cap(tmp_path:Path):
    a=action(action_type='update_budget',entity_type='campaign',entity_id='c1',arguments={'field':'budget'},before={'budget':100},after={'budget':140},spend_delta=20)
...
def test_daily_campaign_create_count_and_budget(tmp_path:Path):
    ...
    for i in range(10):
        ...
        store.add_action('c0',row)
    new=action(action_type='create_campaign',entity_type='campaign',entity_id='newx',arguments={'name':'CODEX-x','state':'PAUSED','budget':10},before={},after={'budget':10},spend_delta=10)
