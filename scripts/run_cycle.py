...
def single_instance():
    ...
    wait_seconds = max(1.0, float(os.environ.get("ADS_CYCLE_LOCK_WAIT_SECONDS", "7200")))
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError(
                        f"timed out after {wait_seconds:g}s waiting for the active Amazon Ads Codex cycle"
                    )
