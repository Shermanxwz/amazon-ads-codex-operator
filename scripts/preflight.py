...
def main() -> int:
    ...
    add(
        (paths.codex_home / "config.toml").exists(),
        "Dedicated production CODEX_HOME configured",
    )
    add(
        (paths.codex_home / "hooks.json").exists(),
        "Owner PreToolUse hooks.json configured",
    )
    add(
        paths.trusted_hook_file.exists(),
        "Frozen Owner-controlled PreToolUse hook deployed",
    )
    ...
    if source_hook.exists() and paths.trusted_hook_file.exists():
        ...
        add(
            digest(source_hook) == digest(paths.trusted_hook_file),
            "Deployed PreToolUse hook matches vetted project source",
        )
    ...
    if (paths.codex_home / "config.toml").exists():
        ...
        add(
            ...
            "Base MCP policy keeps writes approval-gated",
        )
    ...
    if paths.signing_key.exists() and paths.owner_db.exists():
        ...
        add(
            bool(audit.get("ok")),
            f"Owner audit chain valid ({audit.get('entries', 0)} entries)",
        )
        add(
            snap["policy"]["money"].get("owner_daily_spend_ceiling") is not None,
            "owner_daily_spend_ceiling configured",
        )
        profiles = [
            str(value)
            for value in snap["operator"].get("profile_ids", [])
            if str(value) and str(value) != "REPLACE_ME"
        ]
        add(bool(profiles), "At least one real Amazon Ads profile_id configured")
        add(
            str(snap["operator"].get("advertiser_account_id") or "")
            not in {"", "REPLACE_ME"},
            "Advertiser account ID configured",
        )
        add(
            snap["mode"] in {"observe", "paused", "autopilot"},
            f"Owner control mode valid: {snap['mode']}",
        )

    if paths.runtime_db.exists():
        runtime = Store(paths.runtime_db)
        add(
            runtime.integrity_check().get("ok", False),
            "Runtime SQLite integrity check passes",
        )
    else:
    ...
    if codex and (paths.codex_home / "config.toml").exists():
        ...
        compatibility = subprocess.run(
            [sys.executable, str(ROOT / "scripts/check_codex_runtime.py")],
            text=True,
            env=env,
        ...
        add(
            ...
            "Installed Codex satisfies the certified runtime capability contract",
        )
        ...
        add(
            mcp.returncode == 0 and "amazon_ads" in mcp.stdout,
            "amazon_ads MCP configured in dedicated CODEX_HOME",
        )
