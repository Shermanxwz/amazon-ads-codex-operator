...
class Action:
    ...
    def from_dict(cls, row: dict[str, Any]) -> "Action":
        return cls(
            ...
            rollback=dict(row["rollback"]) if isinstance(row.get("rollback"), dict) else None,
            prewrite_observed_at=str(row.get("prewrite_observed_at")) if row.get("prewrite_observed_at") else None,
