...
def run_codex(
    *,
    ...
    try:
        ...
        event_log = output.with_name(output.name + ".events.jsonl")
        event_log.write_text(proc.stdout or "")
        os.chmod(event_log, 0o600)
        if proc.stderr:
            stderr_log = output.with_name(output.name + ".stderr.log")
            stderr_log.write_text(proc.stderr)
            os.chmod(stderr_log, 0o600)
        if proc.returncode != 0:
            raise CodexError(f"codex exec failed ({proc.returncode}): {proc.stderr[-4000:]}")
